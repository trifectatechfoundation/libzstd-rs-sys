        ...
        *fresh0 = (*fresh0 as std::ffi::c_int + 1 as std::ffi::c_int) as U16;
            ...
            == dmersInK.wrapping_add(1 as std::ffi::c_int as U32)
        {
            ...
            *fresh1 = (*fresh1 as std::ffi::c_int - 1 as std::ffi::c_int) as U16;
            if *segmentFreqs.offset(delIndex as isize) as std::ffi::c_int == 0 as std::ffi::c_int {
                activeSegment.score =
...
unsafe extern "C" fn FASTCOVER_checkParameters(
    ...
    if f > FASTCOVER_MAX_F as std::ffi::c_uint || f == 0 as std::ffi::c_int as std::ffi::c_uint {
        return 0 as std::ffi::c_int;
    }
...
unsafe extern "C" fn FASTCOVER_computeFrequency(
    ...
    while i < (*ctx).nbTrainSamples {
        ...
        let currSampleEnd =
            *((*ctx).offsets).offset(i.wrapping_add(1 as std::ffi::c_int as size_t) as isize);
...
pub unsafe extern "C" fn ZDICT_trainFromBuffer_fastCover(
    ...
    parameters.f = if parameters.f == 0 as std::ffi::c_int as std::ffi::c_uint {
        DEFAULT_F as std::ffi::c_uint
    } else {
        parameters.f
    };
    parameters.accel = if parameters.accel == 0 as std::ffi::c_int as std::ffi::c_uint {
