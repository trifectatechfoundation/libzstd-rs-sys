...
unsafe fn ZSTD_count_2segments(
    ...
    let vEnd = if ip.wrapping_offset(mEnd.offset_from(match_0)) < iEnd {
        ip.wrapping_offset(mEnd.offset_from(match_0))
    } else {
        iEnd
    };
...
unsafe fn ZSTD_compressBlock_doubleFast_dictMatchState_generic(
    ...
    let mut ip = istart;
    let mut anchor = istart;
    let endIndex = ((istart as isize - base as isize) as size_t).wrapping_add(srcSize) as u32;
    let prefixLowestIndex = ZSTD_getLowestPrefixIndex(ms, endIndex, (*cParams).windowLog);
    ...
    while ip < ilimit {
        ...
        let curr = (ip as isize - base as isize) as u32;
        ...
        *hashLong.add(h2) = *fresh3;
        ...
        if ip <= ilimit {
            ...
            *hashLong.add(ZSTD_hashPtr(
                ...
                hBitsL,
                8,
            )) = (ip.sub(2) as isize - base as isize) as u32;
            *hashSmall.add(ZSTD_hashPtr(
                base.wrapping_offset(indexToInsert as isize) as *const core::ffi::c_void,
                hBitsS,
                mls,
            ...
            *hashSmall.add(ZSTD_hashPtr(
                ...
                hBitsS,
                mls,
            )) = (ip.sub(1) as isize - base as isize) as u32;
            while ip <= ilimit {
                let current2 = (ip as isize - base as isize) as u32;
