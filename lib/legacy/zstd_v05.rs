...
type decompressionAlgo = unsafe fn(*mut core::ffi::c_void, size_t, *const u8, size_t) -> size_t;
...
#[inline]
unsafe fn BITv05_reloadDStream(bitD: &mut BITv05_DStream_t) -> StreamStatus {
...
#[inline]
fn BITv05_endOfDStream(DStream: &BITv05_DStream_t) -> bool {
...
unsafe fn FSEv05_decompress_usingDTable_generic(
    ...
    while BITv05_reloadDStream(&mut bitD) == StreamStatus::Unfinished && op < olimit {
        ...
        *op.offset(1) = (if fast != 0 {
        ...
        if (FSEv05_MAX_TABLELOG * 4 + 7) as size_t
            > (::core::mem::size_of::<size_t>()).wrapping_mul(8)
            && BITv05_reloadDStream(&mut bitD) > StreamStatus::Unfinished
        {
            op = op.offset(2);
            break;
        }
    ...
    while !(BITv05_reloadDStream(&mut bitD) > StreamStatus::Completed
        || op == omax
        || BITv05_endOfDStream(&bitD) && (fast != 0 || FSEv05_endOfDState(&state1) != 0))
    {
        let fresh5 = op;
        op = op.offset(1);
        *fresh5 = (if fast != 0 {
        ...
        if BITv05_reloadDStream(&mut bitD) > StreamStatus::Completed
            || op == omax
            || BITv05_endOfDStream(&bitD) && (fast != 0 || FSEv05_endOfDState(&state2) != 0)
        {
            break;
        }
        let fresh6 = op;
...
unsafe fn FSEv05_decompress(
    ...
    FSEv05_decompress_usingDTable(dst, maxDstSize, ip, cSrcSize, dt.as_mut_ptr())
}
...
unsafe fn HUFv05_readDTableX2(DTable: *mut u16, src: *const u8, srcSize: size_t) -> size_t {
...
unsafe fn HUFv05_decodeStreamX2(
    ...
    let pStart = p;
    while BITv05_reloadDStream(bitDPtr) == StreamStatus::Unfinished && p <= pEnd.sub(4) {
        if MEM_64bits() != 0 {
            let fresh10 = p;
            p = p.offset(1);
    ...
    while BITv05_reloadDStream(bitDPtr) == StreamStatus::Unfinished && p < pEnd {
        let fresh14 = p;
        p = p.offset(1);
...
unsafe fn HUFv05_decompress1X2(
    ...
    ip = ip.add(errorCode);
    cSrcSize = cSrcSize.wrapping_sub(errorCode);
    HUFv05_decompress1X2_usingDTable(dst, dstSize, ip, cSrcSize, DTable.as_mut_ptr())
}
unsafe fn HUFv05_decompress4X2_usingDTable(
    ...
    end_signal &= BITv05_reloadDStream(&mut bitD1) == StreamStatus::Unfinished;
    end_signal &= BITv05_reloadDStream(&mut bitD2) == StreamStatus::Unfinished;
    end_signal &= BITv05_reloadDStream(&mut bitD3) == StreamStatus::Unfinished;
    end_signal &= BITv05_reloadDStream(&mut bitD4) == StreamStatus::Unfinished;
...
unsafe fn HUFv05_decompress4X2(
    ...
    ip = ip.add(errorCode);
    cSrcSize = cSrcSize.wrapping_sub(errorCode);
    HUFv05_decompress4X2_usingDTable(dst, dstSize, ip, cSrcSize, DTable.as_mut_ptr())
}
...
unsafe fn HUFv05_decodeStreamX4(
    ...
    let pStart = p;
    while BITv05_reloadDStream(bitDPtr) == StreamStatus::Unfinished && p < pEnd.sub(7) {
        if MEM_64bits() != 0 {
            p = p.offset(
    ...
    while BITv05_reloadDStream(bitDPtr) == StreamStatus::Unfinished && p <= pEnd.sub(2) {
        p = p.offset(
            HUFv05_decodeSymbolX4(p as *mut core::ffi::c_void, bitDPtr, dt, dtLog) as isize,
        );
    }
...
unsafe fn HUFv05_decompress4X4_usingDTable(
    ...
    endSignal &= BITv05_reloadDStream(&mut bitD1) == StreamStatus::Unfinished;
    endSignal &= BITv05_reloadDStream(&mut bitD2) == StreamStatus::Unfinished;
    endSignal &= BITv05_reloadDStream(&mut bitD3) == StreamStatus::Unfinished;
    endSignal &= BITv05_reloadDStream(&mut bitD4) == StreamStatus::Unfinished;
...
unsafe fn HUFv05_decompress4X4(
    ...
    HUFv05_decompress4X4_usingDTable(dst, dstSize, ip, cSrcSize, DTable.as_mut_ptr())
}
...
unsafe fn HUFv05_decompress(
    ...
    static decompress: [decompressionAlgo; 2] = [
        HUFv05_decompress4X2
            as unsafe fn(*mut core::ffi::c_void, size_t, *const u8, size_t) -> size_t,
    ...
    if cSrcSize == 1 {
        core::ptr::write_bytes(dst.cast::<u8>(), *cSrc, dstSize);
        return dstSize;
    }
...
unsafe fn ZSTDv05_decodeFrameHeader_Part1(
    ...
    magicNumber = MEM_readLE32(src.cast());
...
unsafe fn ZSTDv05_decompressSequences(
    ...
    if nbSeq != 0 {
        ...
        while BITv05_reloadDStream(&mut seqState.DStream) <= StreamStatus::Completed && nbSeq != 0 {
...
unsafe fn ZSTDv05_decompress_continueDCtx(
    ...
    loop {
        ...
        let cBlockSize =
            ZSTDv05_getcBlockSize(ip, iend.offset_from(ip) as size_t, &mut blockProperties);
...
pub(crate) unsafe fn ZSTDv05_findFrameSizeInfoLegacy(
    ...
    if MEM_readLE32(src.cast()) != ZSTDv05_MAGICNUMBER {
    ...
    loop {
        let cBlockSize = ZSTDv05_getcBlockSize(ip, remainingSize, &mut blockProperties);
        if ERR_isError(cBlockSize) {
    ...
    *cSize = ip.offset_from(src) as size_t;
...
unsafe fn ZSTDv05_decompressContinue(
    ...
    let result = ZSTDv05_decodeFrameHeader_Part2(dctx, dctx.headerBuffer.as_ptr(), dctx.headerSize);
...
unsafe fn ZSTDv05_refDictContent(dctx: &mut ZSTDv05_DCtx, dict: *const u8, dictSize: size_t) {
    dctx.dictEnd = dctx.previousDstEnd;
...
unsafe fn ZSTDv05_loadEntropy(
    ...
    dict = dict.add(offcodeHeaderSize);
    ...
    dict = dict.add(matchlengthHeaderSize);
...
unsafe fn ZSTDv05_decompress_insertDictionary(
    ...
    let magic = MEM_readLE32(dict.cast());
    ...
    while notDone != 0 {
        ...
        match zbc.stage {
            ...
            ZBUFFv05ds_loadHeader => {
                ...
                headerSize_0 =
                    ZSTDv05_getFrameParams(&mut zbc.params, zbc.headerBuffer.as_ptr(), zbc.hPos);
        ...
        if current_block == Block::Flush {
            ...
            let flushedSize = ZBUFFv05_limitCopy(
                op as *mut core::ffi::c_void,
                oend.offset_from(op) as size_t,
                zbc.outBuff.add(zbc.outStart) as *const u8,
                toFlushSize,
            );
