...
fn get_frame_content_size(src: &[u8]) -> u64 {
    ...
    if !get_frame_header_advanced(&mut zfh, src, Format::ZSTD_f_zstd1).is_ok_and(|s| s == 0) {
    ...
    while src.len() >= ZSTD_startingInputLength((*dctx).format) {
        if (*dctx).format == Format::ZSTD_f_zstd1 && is_legacy(src.as_slice()) != 0 {
            ...
            let res = ZSTD_decompressFrame(dctx.as_mut().unwrap(), dst.subslice(..), &mut src)
                .map_err(|err| {
                    if err == Error::prefix_unknown && more_than_one_frame {
                        Error::srcSize_wrong
                    } else {
                        err
                    }
...
fn ZSTD_refDictContent(dctx: &mut ZSTD_DCtx, dict: &[u8]) -> Result<size_t, Error> {
...
fn ZSTD_decompress_insertDictionary(dctx: &mut ZSTD_DCtx, dict: &[u8]) -> Result<size_t, Error> {
